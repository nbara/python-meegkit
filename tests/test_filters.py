...
def generate_simple_signal(npt=40000, fs=100, f1=1.2, f2=10):
    """Generate multi-component data with frequency modulation.
    ...
    dt = 1 / fs
    t = np.arange(npt) * dt
    omega1 = 2 * np.pi * f1  # Frequency of the first component (slow)
    omega2 = 2 * np.pi * f2  # Frequency of the second component (fast)
    amp = 1 + 0.95 * np.cos(omega1 * t)
    p = t + np.sin(omega2 * t)
    s = np.cos(p)
    s += 0.2 * np.cos(2 * p + np.pi / 6)
    s += 0.1 * np.cos(3 * p + np.pi / 3)
    s *= amp
